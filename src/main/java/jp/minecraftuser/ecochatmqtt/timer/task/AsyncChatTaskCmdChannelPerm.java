...
/**
 * タスク別処理分割用 ChannelPerm クラス
 * @author ecolight
 */
public class AsyncChatTaskCmdChannelPerm extends AsyncChatTaskBase {
    
    // シングルトン実装
    private static AsyncChatTaskCmdChannelPerm instance = null;
    public static final AsyncChatTaskCmdChannelPerm getInstance(PluginFrame plg_) {
        if (instance == null) {
            instance = new AsyncChatTaskCmdChannelPerm(plg_);
        }
        return instance;
    }
     ...
     * @param plg_ 
     */
    public AsyncChatTaskCmdChannelPerm(PluginFrame plg_) {
        super(plg_);
    }
    ...
    public void asyncThread(AsyncWorker thread, EcoChatDB db, Connection con, ChatPayload data) throws SQLException {
        ...
        // 変更する値を判定
        boolean old = ch.addPerm;
        ch.addPerm = !ch.addPerm;
        EcoChatDBChannel chdb = (EcoChatDBChannel) plg.getDB("channel");
        chdb.updateChannel(con, ch);
        Utl.sendPluginMessage(plg, data.player, "チャンネルの[addPerm]指定を{0}から{1}に変更しました。", String.valueOf(old), String.valueOf(ch.addPerm));
