 ...
 * @author ecolight
 */
public class AsyncChatTaskCmdChannelLeave extends AsyncChatTaskBase {
    
    // シングルトン実装
    private static AsyncChatTaskCmdChannelLeave instance = null;
    public static final AsyncChatTaskCmdChannelLeave getInstance(PluginFrame plg_) {
        if (instance == null) {
            instance = new AsyncChatTaskCmdChannelLeave(plg_);
        }
        return instance;
    }
     ...
     * @param plg_ 
     */
    public AsyncChatTaskCmdChannelLeave(PluginFrame plg_) {
        super(plg_);
    }
    ...
    public void asyncThread(AsyncWorker thread, EcoChatDB db, Connection con, ChatPayload data) throws SQLException {
        ...
        ch.leaveMessage = sb.toString();
        EcoChatDBChannel chdb = (EcoChatDBChannel) plg.getDB("channel");
        chdb.updateChannel(con, ch);
        Utl.sendPluginMessage(plg, data.player, "チャンネルの[leaveMessage]指定を{0}から{1}に変更しました。", old, ch.leaveMessage);
