...
/**
 * タスク別処理分割用 ConfFlagRangeLocal クラス
 * @author ecolight
 */
public class AsyncChatTaskCmdConfRangeLocal extends AsyncChatTaskBase {
    
    // シングルトン実装
    private static AsyncChatTaskCmdConfRangeLocal instance = null;
    public static final AsyncChatTaskCmdConfRangeLocal getInstance(PluginFrame plg_) {
        if (instance == null) {
            instance = new AsyncChatTaskCmdConfRangeLocal(plg_);
        }
        return instance;
    }
     ...
     * @param plg_ 
     */
    public AsyncChatTaskCmdConfRangeLocal(PluginFrame plg_) {
        super(plg_);
    }
