...
/**
 * タスク別処理分割用 ChannelLeave クラス
 * @author ecolight
 */
public class AsyncChatTaskCmdChannelGoodbye extends AsyncChatTaskBase {
    
    // シングルトン実装
    private static AsyncChatTaskCmdChannelGoodbye instance = null;
    public static final AsyncChatTaskCmdChannelGoodbye getInstance(PluginFrame plg_) {
        if (instance == null) {
            instance = new AsyncChatTaskCmdChannelGoodbye(plg_);
        }
        return instance;
    }
     ...
     * @param plg_ 
     */
    public AsyncChatTaskCmdChannelGoodbye(PluginFrame plg_) {
        super(plg_);
    }
    ...
    public void asyncThread(AsyncWorker thread, EcoChatDB db, Connection con, ChatPayload data) throws SQLException {
        ...
        ch.goodbyeMessage = sb.toString();
        EcoChatDBChannel chdb = (EcoChatDBChannel) plg.getDB("channel");
        chdb.updateChannel(con, ch);
        Utl.sendPluginMessage(plg, data.player, "チャンネルの[goodbyMessage]指定を{0}から{1}に変更しました。", old, ch.goodbyeMessage);
