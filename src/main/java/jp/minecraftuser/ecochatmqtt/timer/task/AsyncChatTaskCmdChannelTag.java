...
/**
 * タスク別処理分割用 ChannelTag クラス
 * @author ecolight
 */
public class AsyncChatTaskCmdChannelTag extends AsyncChatTaskBase {
    
    // シングルトン実装
    private static AsyncChatTaskCmdChannelTag instance = null;
    public static final AsyncChatTaskCmdChannelTag getInstance(PluginFrame plg_) {
        if (instance == null) {
            instance = new AsyncChatTaskCmdChannelTag(plg_);
        }
        return instance;
    }
     ...
     * @param plg_ 
     */
    public AsyncChatTaskCmdChannelTag(PluginFrame plg_) {
        super(plg_);
    }
    ...
    public void asyncThread(AsyncWorker thread, EcoChatDB db, Connection con, ChatPayload data) throws SQLException {
        ...
        // 禁止チャンネル名を判定
        if (Channel.checkIgnoreTagName(data.param[0])) {
            Utl.sendPluginMessage(plg, data.player, "指定チャンネルタグは作成が禁止されています[" + data.param[0] + "]");
            return;
        }
        ...
        Utl.sendPluginMessage(plg, data.player, "チャンネルの[tag]を{0}から{1}に変更しました。", old, ch.tag);
