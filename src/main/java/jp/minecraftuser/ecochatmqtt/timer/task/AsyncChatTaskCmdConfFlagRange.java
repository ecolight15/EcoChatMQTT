...
/**
 * タスク別処理分割用 ConfFlagRange クラス
 * @author ecolight
 */
public class AsyncChatTaskCmdConfFlagRange extends AsyncChatTaskBase {
    
    // シングルトン実装
    private static AsyncChatTaskCmdConfFlagRange instance = null;
    public static final AsyncChatTaskCmdConfFlagRange getInstance(PluginFrame plg_) {
        if (instance == null) {
            instance = new AsyncChatTaskCmdConfFlagRange(plg_);
        }
        return instance;
    }
     ...
     * @param plg_ 
     */
    public AsyncChatTaskCmdConfFlagRange(PluginFrame plg_) {
        super(plg_);
    }
    ...
    public void asyncThread(AsyncWorker thread, EcoChatDB db, Connection con, ChatPayload data) throws SQLException {
        ...
        // 変更する値を判定
        boolean old = us.infoNotifyCount;
        us.infoNotifyCount = !us.infoNotifyCount;
        usdb.updateUser(con, us);
        Utl.sendPluginMessage(plg, data.player, "infoNotifyCount設定を{1}から{2}に変更しました。", String.valueOf(old), String.valueOf(us.infoNotifyCount));
    ...
    public void mainThread(AsyncWorker thread, ChatPayload data) {
        if (!data.result) {
            Utl.sendPluginMessage(plg, data.player, "プレイヤーのinfoNotifyCount設定の変更に失敗しました");
        }
    }
