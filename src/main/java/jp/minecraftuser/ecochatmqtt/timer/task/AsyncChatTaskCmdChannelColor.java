...
/**
 * タスク別処理分割用 ChannelColor クラス
 * @author ecolight
 */
public class AsyncChatTaskCmdChannelColor extends AsyncChatTaskBase {
    
    // シングルトン実装
    private static AsyncChatTaskCmdChannelColor instance = null;
    public static final AsyncChatTaskCmdChannelColor getInstance(PluginFrame plg_) {
        if (instance == null) {
            instance = new AsyncChatTaskCmdChannelColor(plg_);
        }
        return instance;
    }
     ...
     * @param plg_ 
     */
    public AsyncChatTaskCmdChannelColor(PluginFrame plg_) {
        super(plg_);
    }
    ...
    public void asyncThread(AsyncWorker thread, EcoChatDB db, Connection con, ChatPayload data) throws SQLException {
        ...
        if (col == null) {
            Utl.sendPluginMessage(plg, data.player, "チャンネルタグ色に指定した文字列が判別できませんでした");
            data.result = false;
            return;
        }
        String old = chconf.color;
        chconf.color = col.name();
        
        chconfdb.updateChannelConf(con, chconf);
        Utl.sendPluginMessage(plg, data.player, "チャンネルタグの[color]指定を{0}から{1}に変更しました。", old, chconf.color);
