...
/**
 * タスク別処理分割用 ChannelList クラス
 * @author ecolight
 */
public class AsyncChatTaskCmdChannelList extends AsyncChatTaskBase {
    
    // シングルトン実装
    private static AsyncChatTaskCmdChannelList instance = null;
    public static final AsyncChatTaskCmdChannelList getInstance(PluginFrame plg_) {
        if (instance == null) {
            instance = new AsyncChatTaskCmdChannelList(plg_);
        }
        return instance;
    }
     ...
     * @param plg_ 
     */
    public AsyncChatTaskCmdChannelList(PluginFrame plg_) {
        super(plg_);
    }
    ...
    public void asyncThread(AsyncWorker thread, EcoChatDB db, Connection con, ChatPayload data) throws SQLException {
        ...
        // 変更する値を判定
        boolean old = ch.listed;
        ch.listed = !ch.listed;
        EcoChatDBChannel chdb = (EcoChatDBChannel) plg.getDB("channel");
        chdb.updateChannel(con, ch);
        Utl.sendPluginMessage(plg, data.player, "チャンネルの[listed]指定を{0}から{1}に変更しました。", String.valueOf(old), String.valueOf(ch.listed));
