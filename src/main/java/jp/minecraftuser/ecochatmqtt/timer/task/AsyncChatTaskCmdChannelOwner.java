...
/**
 * タスク別処理分割用 ChannelOwner クラス
 * @author ecolight
 */
public class AsyncChatTaskCmdChannelOwner extends AsyncChatTaskBase {
    
    // シングルトン実装
    private static AsyncChatTaskCmdChannelOwner instance = null;
    public static final AsyncChatTaskCmdChannelOwner getInstance(PluginFrame plg_) {
        if (instance == null) {
            instance = new AsyncChatTaskCmdChannelOwner(plg_);
        }
        return instance;
    }
     ...
     * @param plg_ 
     */
    public AsyncChatTaskCmdChannelOwner(PluginFrame plg_) {
        super(plg_);
    }
    ...
    public void asyncThread(AsyncWorker thread, EcoChatDB db, Connection con, ChatPayload data) throws SQLException {
        ...
        if (chus_target == null) {
            Utl.sendPluginMessage(plg, data.player, "指定ユーザはチャンネルに参加していません");
            data.result = false;
            return;
        }
        
        // 変更する値を判定
        boolean old = chus_target.owner;
        chus_target.owner = !chus_target.owner;

        // チャンネルユーザ情報を更新する
        EcoChatDBChannelUser chusdb = (EcoChatDBChannelUser) plg.getDB("channel_user");
        chusdb.updateChannelUser(con, chus_target);
        ...
        // Owner追加メッセージの送信
        if (old) {
            ch.sendPlayerBroadcast(plg, target.uuid, "チャンネル[" + ch.tag + "]の管理者グループから削除されました");
            Utl.sendPluginMessage(plg, data.player, "指定プレイヤーをアクティブチャンネルの管理者グループから削除しました");
        } else {
            ch.sendPlayerBroadcast(plg, target.uuid, "チャンネル[" + ch.tag + "]の管理者グループに追加されました");
            Utl.sendPluginMessage(plg, data.player, "指定プレイヤーをアクティブチャンネルの管理者グループに追加しました");
        }
