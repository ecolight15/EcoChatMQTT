 ...
 * @author ecolight
 */
public class ConfRangeLocalCommand extends CommandFrame {
     ...
     * @param name_ コマンド名
     */
    public ConfRangeLocalCommand(PluginFrame plg_, String name_) {
        super(plg_, name_);
        setAuthBlock(true);
        setAuthConsole(true);
    }
    ...
    public String getPermissionString() {
        return "ecochatmqtt.chat.conf.range.local";
    }
    ...
    public boolean worker(CommandSender sender, String[] args) {
        // /conf flag range local
        // パラメータチェック:0のみ
        if (!checkRange(sender, args, 0, 0)) return true;

        AsyncWorker worker = (AsyncWorker) plg.getPluginTimer("worker");
        ChatPayload data = new ChatPayload(plg, (Player) sender, ChatPayload.Type.CMD_CONF_RANGE_LOCAL);
