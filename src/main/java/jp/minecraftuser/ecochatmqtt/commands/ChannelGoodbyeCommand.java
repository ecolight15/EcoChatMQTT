 ...
 * @author ecolight
 */
public class ChannelGoodbyeCommand extends CommandFrame {
     ...
     * @param name_ コマンド名
     */
    public ChannelGoodbyeCommand(PluginFrame plg_, String name_) {
        super(plg_, name_);
        setAuthBlock(true);
        setAuthConsole(true);
    }
    ...
    public String getPermissionString() {
        return "ecochatmqtt.chat.channel.goodbye";
    }
    ...
    public boolean worker(CommandSender sender, String[] args) {
        // /channel goodbye <message>
        // パラメータチェック:1以上
        if (!checkRange(sender, args, 1, -1)) return true;

        AsyncWorker worker = (AsyncWorker) plg.getPluginTimer("worker");
        ChatPayload data = new ChatPayload(plg, (Player) sender, ChatPayload.Type.CMD_CHANNEL_GOODBYE);
