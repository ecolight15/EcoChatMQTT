 ...
 * @author ecolight
 */
public class ConfMuteCommand extends CommandFrame {
     ...
     * @param name_ コマンド名
     */
    public ConfMuteCommand(PluginFrame plg_, String name_) {
        super(plg_, name_);
        setAuthBlock(true);
        setAuthConsole(true);
    }
    ...
    public String getPermissionString() {
        return "ecochatmqtt.chat.conf.mute";
    }
    ...
    public boolean worker(CommandSender sender, String[] args) {
        // /conf mute <player name>
        // パラメータチェック:1以上
        if (!checkRange(sender, args, 1, -1)) return true;

        AsyncWorker worker = (AsyncWorker) plg.getPluginTimer("worker");
        ChatPayload data = new ChatPayload(plg, (Player) sender, ChatPayload.Type.CMD_CONF_MUTE);
