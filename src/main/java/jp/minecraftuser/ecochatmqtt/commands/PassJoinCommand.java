 ...
 * @author ecolight
 */
public class PassJoinCommand extends CommandFrame {
     ...
     * @param name_ コマンド名
     */
    public PassJoinCommand(PluginFrame plg_, String name_) {
        super(plg_, name_);
        setAuthBlock(true);
        setAuthConsole(true);
    }
    ...
    public String getPermissionString() {
        return "ecochatmqtt.chat.passjoin";
    }
    ...
    public boolean worker(CommandSender sender, String[] args) {
        // /passjoin <channel TAG> <password>
        // パラメータチェック:2のみ
        if (!checkRange(sender, args, 2, 2)) return true;

        AsyncWorker worker = (AsyncWorker) plg.getPluginTimer("worker");
        ChatPayload data = new ChatPayload(plg, (Player) sender, ChatPayload.Type.CMD_PASSJOIN);
