 ...
 * @author ecolight
 */
public class PmHistoryCommand extends CommandFrame {
     ...
     * @param name_ コマンド名
     */
    public PmHistoryCommand(PluginFrame plg_, String name_) {
        super(plg_, name_);
        setAuthBlock(true);
        setAuthConsole(true);
    }
    ...
    public String getPermissionString() {
        return "ecochatmqtt.chat.pmhistory";
    }
    ...
    public boolean worker(CommandSender sender, String[] args) {
        // /pmhistory [yyyy/mm/dd] [page]
        // /pmhistory [page]
        // パラメータチェック:0-2
        if (!checkRange(sender, args, 0, 2)) return true;

        AsyncWorker worker = (AsyncWorker) plg.getPluginTimer("worker");
        ChatPayload data = new ChatPayload(plg, (Player) sender, ChatPayload.Type.CMD_PMHISTORY);
