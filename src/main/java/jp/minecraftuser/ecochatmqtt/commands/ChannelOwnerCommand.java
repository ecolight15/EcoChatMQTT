 ...
 * @author ecolight
 */
public class ChannelOwnerCommand extends CommandFrame {
     ...
     * @param name_ コマンド名
     */
    public ChannelOwnerCommand(PluginFrame plg_, String name_) {
        super(plg_, name_);
        setAuthBlock(true);
        setAuthConsole(true);
    }
    ...
    public String getPermissionString() {
        return "ecochatmqtt.chat.channel.owner";
    }
    ...
    public boolean worker(CommandSender sender, String[] args) {
        // /channel owner <player name1> [player name2]...
        // パラメータチェック:1以上
        if (!checkRange(sender, args, 1, -1)) return true;

        AsyncWorker worker = (AsyncWorker) plg.getPluginTimer("worker");
        ChatPayload data = new ChatPayload(plg, (Player) sender, ChatPayload.Type.CMD_CHANNEL_OWNER);
